...
def _render_beginner_output(language: str) -> str:
    ...
    if language == "zh-CN":
        return "\n".join(
            [
                ...
                "## 市场预期与实际结果",
                "",
                "- 如果本地数据缺少市场或用户结果证据，系统会说明证据不足，而不是推断收益或损失。",
                "",
                "## 接下来可以关注什么",
                "",
                "- 哪些供应商、客户或地区暴露度最高。",
    ...
    if language == "zh-TW":
        return "\n".join(
            [
                ...
                "## 市場預期與實際結果",
                "",
                "- 如果本地資料缺少市場或使用者結果證據，系統會說明證據不足，而不是推斷收益或損失。",
                "",
                "## 接下來可以關注什麼",
                "",
                "- 哪些供應商、客戶或地區暴露度最高。",
    ...
    return "\n".join(
            ...
            "",
            "- If market or user outcome evidence is missing, the system states that the local dataset does not contain enough evidence rather than infer gains or losses.",
            "",
            "## What To Watch Next",
            "",
